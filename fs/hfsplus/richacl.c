...
static uint32_t convert_rights[32] = {
	...
	NFS4_ACE_DELETE,		/* HFSPLUS_VNODE_DELETE */
	NFS4_ACE_APPEND_DATA,		/* HFSPLUS_VNODE_APPEND_DATA */
	NFS4_ACE_DELETE_CHILD,		/* HFSPLUS_VNODE_DELETE_CHILD */
	NFS4_ACE_READ_ATTRIBUTES,	/* HFSPLUS_VNODE_READ_ATTRIBUTES */
	NFS4_ACE_WRITE_ATTRIBUTES,	/* HFSPLUS_VNODE_WRITE_ATTRIBUTES */
	NFS4_ACE_READ_NAMED_ATTRS,	/* HFSPLUS_VNODE_READ_EXTATTRIBUTES */
	NFS4_ACE_WRITE_NAMED_ATTRS,	/* HFSPLUS_VNODE_WRITE_EXTATTRIBUTES */
	NFS4_ACE_READ_ACL,		/* HFSPLUS_VNODE_READ_SECURITY */
	NFS4_ACE_WRITE_ACL,		/* HFSPLUS_VNODE_WRITE_SECURITY */
	NFS4_ACE_WRITE_OWNER,		/* HFSPLUS_VNODE_TAKE_OWNERSHIP */
	0,				/* 14 */
	0,				/* 15 */
	0,				/* 16 */
	0,				/* 17 */
	...
	0,				/* 19 */
	0,				/* 20 */
	NFS4_ACE_MASK_ALL,		/* HFSPLUS_ACE_GENERIC_ALL */
	NFS4_ACE_GENERIC_EXECUTE,	/* HFSPLUS_ACE_GENERIC_EXECUTE */
	NFS4_ACE_GENERIC_WRITE,		/* HFSPLUS_ACE_GENERIC_WRITE */
	NFS4_ACE_GENERIC_READ,		/* HFSPLUS_ACE_GENERIC_READ */
	0,				/* 25 */
	0,				/* 26 */
	0,				/* 27 */
	0,				/* 28 */
...
static uint32_t hfsplus_ace_rights_to_nfsv4(struct hfsplus_acl_entry *hfs_ace)
{
...
static u8 convert_flags[16] = {
	...
	0,				/* 2 */
	0,				/* 3 */
	0,				/* HFSPLUS_ACE_INHERITED */
	NFS4_ACE_FILE_INHERIT_ACE,	/* HFSPLUS_ACE_FILE_INHERIT */
	NFS4_ACE_DIRECTORY_INHERIT_ACE,	/* HFSPLUS_ACE_DIRECTORY_INHERIT */
	NFS4_ACE_NO_PROPAGATE_INHERIT_ACE, /* HFSPLUS_ACE_LIMIT_INHERIT */
	NFS4_ACE_INHERIT_ONLY_ACE,	/* HFSPLUS_ACE_ONLY_INHERIT */
	NFS4_ACE_SUCCESSFUL_ACCESS_ACE_FLAG, /* HFSPLUS_ACE_SUCCESS */
	NFS4_ACE_FAILED_ACCESS_ACE_FLAG, /* HFSPLUS_ACE_FAILURE */
	0,				/* 11 */
	0,				/* 12 */
	0,				/* 13 */
	0,				/* 14 */
...
static uint32_t hfsplus_ace_flags_to_nfsv4(struct inode *inode,
...
static inline
void hfsplus_ace_extract_id(struct inode *inode,
					struct hfsplus_acl_entry *hfs_ace,
					struct nfs4_ace *ace)
{
	if (is_owner_ace(hfs_ace))
		ace->who_uid = i_uid_read(inode);
	else if (is_group_owner_ace(inode, hfs_ace))
		ace->who_gid = i_gid_read(inode);
	else
		ace->who_uid = extract_uid_from_ace(hfs_ace);
}
						...
						struct hfsplus_filesec *filesec)
{
	...
	struct hfsplus_acl_entry *hfs_ace, *end;

	hfs_dbg(ACL_MOD, "[%s]: filesec %p, entries_count %u\n",
			__func__, filesec, entries_count);

	hfs_ace = hfsplus_acl_record_from_xattr(value, size);
	if (unlikely(IS_ERR(hfs_ace)))
		return HFS_ERR_DBG(ACL_MOD, (int)PTR_ERR(hfs_ace));
	end = hfs_ace + acl_entries_count;
...
hfsplus_richacl_from_xattr(struct inode *inode,
				...
				size_t size)
{
	...
	nfs4acl = hfsplus_acl_to_nfsv4(inode, filesec);
	if (unlikely(nfs4acl == NULL))
		return ERR_PTR(HFS_ERR_DBG(ACL_MOD, -ENOMEM));
	else if (unlikely(IS_ERR(nfs4acl)))
		return ERR_PTR(HFS_ERR_DBG(ACL_MOD, (int)PTR_ERR(nfs4acl)));

	acl = nfs4_acl_nfsv4_to_richacl(nfs4acl);
